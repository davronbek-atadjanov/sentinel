...
import { TrendingUp, Brain, AlertTriangle, Shield, BarChart3 } from "lucide-react";

const predictions = [
  { category: "SQL Injection", current: 12, predicted: 18, trend: "+50%", risk: "High", confidence: 89 },
  { category: "XSS Attacks", current: 8, predicted: 6, trend: "-25%", risk: "Medium", confidence: 92 },
  { category: "Auth Bypass", current: 3, predicted: 7, trend: "+133%", risk: "Critical", confidence: 78 },
  { category: "SSRF", current: 1, predicted: 4, trend: "+300%", risk: "High", confidence: 71 },
  { category: "Misconfig", current: 15, predicted: 12, trend: "-20%", risk: "Low", confidence: 95 },
];
  ...
  return (
    <div>
      <PageHeader
        ...
        description="ML-powered vulnerability trend forecasting. Predict emerging threats and prioritize security investments based on data-driven insights."
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        {[
          { label: "Prediction Accuracy", value: "87%", icon: Brain },
          { label: "Threat Vectors Tracked", value: "142", icon: TrendingUp },
          { label: "Risk Score (30d)", value: "High", icon: AlertTriangle, color: "text-sentinel-tertiary" },
        ].map((s) => (
      ...
      <div className="bg-surface-low rounded-xl p-6 border-ghost mb-8">
        <h3 className="font-bold text-white mb-6">30-Day Vulnerability Forecast</h3>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="text-[10px] text-[hsl(215,15%,45%)] uppercase tracking-widest font-bold border-b border-[hsl(222,20%,12%,0.2)]">
                ...
                <th className="px-5 py-3">Predicted (30d)</th>
      ...
      <div className="bg-surface-low rounded-xl p-6 border-ghost border-l-4 border-l-sentinel-tertiary">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className="text-sm font-bold text-white mb-1">AI Prediction Alert</h4>
            <p className="text-xs text-[hsl(215,15%,55%)] leading-relaxed">
              The model predicts a <span className="text-sentinel-tertiary font-bold">133% increase</span> in authentication bypass attempts over the next 30 days. 
              This correlates with recently published exploit kits targeting OAuth2 implementations. Recommend immediate review of all auth endpoints.
            </p>
          </div>
        </div>
      </div>
