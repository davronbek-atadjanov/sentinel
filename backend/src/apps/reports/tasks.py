...
def _generate_pdf(report, data: dict) -> io.BytesIO:
    ...
    elements.append(Paragraph(f"Sentinel Security Report", title_style))
    ...
    if data.get("scan"):
        meta_data.append(["Target URL:", data["scan"].get("target_url", "N/A")])
        meta_data.append(["Scan Type:", data["scan"].get("scan_type", "N/A")])
    ...
    if vuln_details:
        ...
        for i, vuln in enumerate(vuln_details[:20], 1):
            ...
            if vuln.get("remediation"):
                elements.append(Paragraph(f"&nbsp;&nbsp;Fix: {vuln['remediation']}", styles["Normal"]))
