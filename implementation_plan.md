...
| 6 | `notifications` | Ichki tizim ogohlantirishlari (NEW) |
...
## Faza 2.1: Skanlarni Frontend UI ga Ulash (React Query)

> [!IMPORTANT]
> Skanerlash backend qismi qilingan, endi `frontend/src/pages/scans/` dagi fayllarda soxta (mock) ma'lumotlarni o'chirib, React-Query va Backend API ga ulangadi. 
> Barcha o'zgarishlar real-time asinxron operatsiyalarni hisobga oladi (masalan, scans ro'yxati `refetchInterval` bilan har 5 soniyada yangilinib turadi to progressni ko'rish uchun).

### 2.1.1 `frontend/src/services/scans.service.ts`
- Mavjud `getScans`, `createScan`, `getScan` metodlari tekshiriladi.
- Kerak bo'lsa Skan turlari (Scan interface) aniq typization bilan moslashtiriladi.

### 2.1.2 `ScansListPage.tsx`
#### [MODIFY] [ScansListPage.tsx](file:///c:/Users/Atajanov%20Davronbek/Desktop/individual_loyiha/frontend/src/pages/scans/ScansListPage.tsx)
- `const activeScans = [...]` kabi o'zgarmas ma'lumotlar olib tashlanadi.
- `useQuery({ queryKey: ["scans"], queryFn: ScansService.getScans, refetchInterval: 5000 })` ishlatiladi (faqat active skanlar bor bo'lsagina).
- Backenddan kelgan sanalar `date-fns` formatga o'tkaziladi.
- Jadvaldagi satrlar dinamik map qilinadi. Progress Bar `scan.progress` foiziga to'g'ridan-to'g'ri bog'lanadi.

### 2.1.3 Yangi Skan Boshlash Modal / Sahifasi
#### [MODIFY] [ScanConfigPage.tsx](file:///c:/Users/Atajanov%20Davronbek/Desktop/individual_loyiha/frontend/src/pages/scans/ScanConfigPage.tsx) yaki `NewScanModal`
- Foydalanuvchidan *Target URL* (domen) so'raladi.
- Skaner turi (`FULL`, `QUICK`, `OWASP`) tanlanadi.
- `useMutation` yordamida `ScansService.createScan` chaqiriladi.
- Yuborilgandan so'ng, `queryClient.invalidateQueries` qilib Skanlar ro'yxatiga qaytariladi (yoki avtomatik ro'yxat yangilanadi) va Sonner toast orqali xabar chiqadi.

### 2.1.4 `ScanDetailsPage.tsx` yoki Xulosa qismi
- Alohida bitta skan bosilganda `/api/v1/scans/{id}/` dan olinadigan podrobniy ma'lumot (shu skan davomida topilgan Vuln'lar ro'yxati va pie chart qilib ko'rsatish) bog'lanadi.
- ZAP / Nuclei o'tkazgan har bir vulnerability'ning summary'si `results_summary` state'ga solinadi.

### Open Questions (Siz tasdiqlashingiz lozim):
1. Hozirgi `ScanConfigPage.tsx` ni Form qilib alohida saqlab o'shani uliymi yoki kichik Modal ishlatamizmi "New Scan" uchun? Modal ancha qulay va zamonaviy UX beradi, lekin Config sahifa ko'proq detaylli imkoniyatlar (Cookies, Auth tokens kiritish) berish imkoniga ega.
2. Skan qilinayotgan paytida 5 sekundlik `refetchInterval` brauzerda qanday refresh tezligida bo'lishini xohlaysiz? 5 s ideal bo'ladi.
...
## Faza 10: Integratsiyalar va Webhook Arxitekturasi (Batafsil Reja)

Ushbu modul Sentinel tizimining markaziy qismi bo'lib qoladi, bu modul orqali tizim tashqi platformalar (Slack, Jira, GitHub Actions, PagerDuty, Jenkins, va boshqalar) bilan xavfsiz va asinxron tarzda aloqa o'rnatadi. Foydalanuvchilar o'zlari xohlagan tashqi URL manziliga "Webhook" yaratishi va Sentinelda sodir bo'ladigan voqealarni (events) real vaqtda yuborishi mumkin.

Biz rejamizni mantiqan Frontend va Backend hamda Worker qatlamlariga ajratamiz, hamda yuqori mashtablilik (Scalability) va Kafolatlilik (Reliability) mezonlarini inobatga olgan holda aniq qadamlarni belgilaymiz.

---

### 10.1 Backend: Ma'lumotlar Bazasi Arxitkekturasi (Models)

Biz Django tizimida mutlaqo yangi app – `apps/integrations` yaratamiz.

#### 10.1.1 `Webhook` Modeli (`apps/integrations/models/webhooks.py`)
Webhook jadvallari orqali mijoz qachon qaysi event yuborilishini belgilab beradi.

- `id`: UUIDField, primary key.
- `user`: ForeignKey -> User (kimga tegishli).
- `name`: CharField (webhookning nomi, masalan "Production Log Relay").
- `url`: URLField (jo'natiladigan tashqi manzil).
- `secret_key`: CharField (payload imzosini (signature) tekshirish uchun, xavfsizlik parametri, default=random).
- `events`: ArrayField yoki JSONField (qaysi voqealar obuna bo'lgan. Masalan `["Scan.Finished", "Vulnerability.Critical"]`).
- `is_active`: BooleanField (default True).
- `created_at`, `updated_at`: Datetime fields.

#### 10.1.2 `WebhookDelivery` (Audit) Modeli (`apps/integrations/models/deliveries.py`)
Har bir yuborilgan HTTP POST zaprosini yozib boramiz, bu foydalanuvchiga API oynasida ulanish to'g'ri bo'layotganini kuzatish imkonini (200 OK, 404 Not Found) beradi.

- `id`: UUIDField.
- `webhook`: ForeignKey -> Webhook.
- `event_type`: CharField.
- `request_payload`: JSONField (qanday ma'lumot jo'natilgan).
- `response_status_code`: IntegerField (masalan 200, 500).
- `response_body`: TextField (qarshi tomondan kelgan xatolik kodi yoki JSON javob).
- `duration_ms`: IntegerField (so'rov qancha vaqt davom etdi).
- `created_at`: Datetime field.

#### 10.1.3 `ExternalIntegration` Modeli (`apps/integrations/models/external.py`)
Jira yoki Slack kabi tayyor "Native" ulashmalar uching xizmat qiladi. Endi webhookdan ko'ra OAuth yoki API Token larni saqlash uchin:

- `id`: UUIDField.
- `user`: ForeignKey -> User.
- `provider`: CharField (choices: "slack", "jira", "github").
- `credentials`: JSONField (shifrlangan, access_token va refresh_tokenlar qayd etiladi).
- `metadata`: JSONField (Slack channel ID yoki Jira Project Key).
- `status`: CharField (CONNECTED, DISCONNECTED).

---

### 10.2 Backend: API Endpoints (Serializers & Views)

`apps/integrations/api/v1/urls.py` router orqali quyidagi API endpointlarini chiqaramiz:

#### Webhook Boshqaruvi
- **GET `/api/v1/integrations/webhooks/`**: Foydalanuvchiga o'zining barcha webhooklarini qaytaradi.
- **POST `/api/v1/integrations/webhooks/`**: Yangi webhook qo'shish. Mijozdan faqat `name`, `url` va `events` massivini oladi. Secret avto-generatsiya qilinadi.
- **GET `/api/v1/integrations/webhooks/<id>/`**: Webhook sirlarini, qo'shimcha so'rovlar tarixini qaytaradi.
- **PUT/PATCH `/api/v1/integrations/webhooks/<id>/`**: URL yoki obuna bo'lingan eventlarni o'zgartirish, yoqish/o'chirish.
- **DELETE `/api/v1/integrations/webhooks/<id>/`**: Webhookni butunlay o'chirish.

#### Yetkazib berish (Delivery Logs)
- **GET `/api/v1/integrations/webhooks/<id>/deliveries/`**: (Pagination bilan) shu maxsus webhook bo'yicha ketgan oxirgi so'rovlar loglarini, javob statuslarini qaytarish (Frontenddagi Status krugini Yashil/Qizil qilish uchain zarur).
- **POST `/api/v1/integrations/webhooks/<id>/ping/`**: Shu zahoti test sifatida namunaviy ma'lumot yuborish.

#### Tashqi Xizmatlar
- **GET `/api/v1/integrations/apps/`**: Jira, Slack kabilar ulanish holatini qaytaradi.

---

### 10.3 Background Worker (Celery Webhook Deliverer)

Biz hech qachon webhook'ni asosiy HTTP thread (API so'rovi jarayoni) ichidan yubormaymiz, aks holda u yiqilsa foydalanuchi serverimiz sekinligini xis qiladi. Worker tizimida quyidagicha mexanizm quriladi:

#### 10.3.1 Signal va Trigger
Djangoning Post-Save yoki Scan-Completed signalida biz:
```python
@receiver(scan_completed)
def hook_scan_completion(sender, instance, **kwargs):
    # Foydalanuvchining shunga tegishli barcha aktiv webhooklarini qidiradi
    webhooks = Webhook.objects.filter(user=instance.user, is_active=True, events__contains="Scan.Finished")
    for hook in webhooks:
        # Background taskga payload qilib berib yuboramiz
        deliver_webhook_task.delay(hook.id, "Scan.Finished", generate_scan_payload(instance))
```

#### 10.3.2 Celery Task (`apps/integrations/tasks.py`)
`deliver_webhook_task(webhook_id, event_type, payload)` funksiyasi yoziladi. Uning majburiyatlari:
1. `requests.post()` orqali `timeout=5s` bilanoq ma'lumotni tashqi mijoz `url` ga json shaklida yuborish.
2. Xavfsizlik signaturasi: Payloadni `HMAC-SHA256` yordamida `secret_key` dan shifrlab Header ga qo'shib yuboradi (masalan `X-Sentinel-Signature`), mijoz o'zida kelgan ma'lumot chindan Sentinel'dan ekaniga amin bo'lishi ucin.
3. **Qayta Urinish (Retry Strategy):** Agar tashqi server qulagan bo'lsa (Masalan 502 Bad Gateway qaytsa), Celery `@task(bind=True, max_retries=3)` xususiyatidan foydalanib, Exponensial tarzda (Avvat 5 soniya, keyin 30 sek, keyin 2 minutdan so'ng) zaprosni davom ettirish logikasi kiritiladi.
4. Natija qanday bo'lishidan qat'iy nazar `WebhookDelivery` jadvaliga yangi qator yozib qo'yadi. Agar oxirgi status 4xx yoki 5xx bo'lsa, ushbu Webhookning avtomatik ravishda Notification tizimiga (CRITICAL webhook failed) ogohlantirish tushuriladi.

#### 10.3.3 Payload Strukturalari (Standard Interfaces)
Tashqariga chiquvchi har qanday ma'lumot quyidagi Standart JSON shaklida o'raladi:
```json
{
  "event_id": "uuid",
  ...
  "data": {
    ...
    "severity": "CRITICAL",
    "target": "api.prod.local",
    "url": "https://sentinel.local/vuln/v-1234"
  }
}
```
...
### 10.4 Frontend: Oynaviy Dasturni (UI) Ulash (`IntegrationsPage.tsx`)

Mijozga Figma orqali chizilgan `IntegrationsPage.tsx` xozirgi paytda faqat static o'zgaruvchilarni (`const logs=...`) va layoutni tutib turibdi. Endi API tayyor bo'lgach buni tiriltiramiz.

#### 10.4.1 React Query Hooks Dasturlash
`src/hooks/queries/useIntegrations.ts` ni yaratish, unda:
- `useWebhooks()` - barcha aktiv webhooklarni tortib oladi.
- `useCreateWebhook(mutation_fn)` - React Query Mutation, yangi URL va event larni backend'ga yuborish.
- `useDeleteWebhook()` - Ro'yxatdan olib tashlash.
- `useDeliveries(webhookId)` - Agar foydalanuvchi webhook ichiga kirib statusini o'rganmoqchi bo'lsa loglarini tortib beradi.

#### 10.4.2 UI da bog'lash jarayoni
**1. "CI/CD Pipelines", "Alerting", "Ticketing" kartochkalari:**
Bu qismlar qanday bo'lsa shundayligicha dizaynda turadi (GitHub Actions, Slack, Jira). Holatida e'tibor bersangiz `CONNECTED` yozuvi bor edi.
Uni biz `ExternalIntegration` modeliga REST API tushirib:
```typescript
const isSlackConnected = externalApps.some(app => app.provider === 'slack' && app.status === 'CONNECTED');
```
deb shartli rendering qilamiz. Hozircha MVP versiyada "Connect Service" tugmasi bosilsa faqat oddiy modal ochish bilan cheklaymiz.

**2. "Webhook Management" Bo'limi:**
Sahifa tagidagi bo'limni to'liq backend ro'yxat bilan sinxronlashish eng katta vazifadir:

- **Webhooks Table**: Har bir row (qator) orqali `webhook.name`, `webhook.url`, uzatilgan `events` ko'rsatiladi.
- **Status indikatori**: (Rasmdagi 200 OK kabi). Backendagi `webhook.latest_delivery_status` nomli proxy property orqali oxirgi status tortib olinib, yashil yokida qizil dumaloqqa qo'shib yoziladi.
- **Add Endpoint Modali**: "Add Endpoint" bosilganda yangi forma ochiladi. Forma ichida foydalanuvchi quyidagilarni to'ldiradi:
  - Nomi (text)
  - Endpoint URL (text)
  - Qaysi voqealar olinsin? (Multiple Checkbox: Scan Started, Scan Finished, Vuln Critical, Vuln High kabi)
Formani Save tugmasini bosganda React-Query mutate qilib API'ga ketadi (va `toast.success` orqali muvoffaqiyat qayd etiladi jadval darhol yangilanadi).

#### 10.4.3 Type xavfsizligini kiritish
`src/types/integrations.types.ts` yaratamiz:
```typescript
export interface Webhook {
  ...
  url: string;
  events: string[];
  last_status: string; // "200 OK", "500 Error", "Never triggered"
  secret_key: string;
  is_active: boolean;
}
...
export interface ExternalApp {
  ...
  details: string; // masalan "Channels: #sec-ops"
}
```

---

### 10.5 Amaliy Sinov va Deployment Strategiyasi

Loyihaning test qismi o'ta muhim bo'lgani ucin quyidagilardan o'tamiz:
1. **Mocking External Server:** Integratsiyani o'zimiz yaratish jarayonida `webhook.site` kabi tayyor xizmatlarning URL ini ulab ko'ramiz.
2. **Celery Worker Logs:** Backendda `celery -A core worker -l info` orqali kuzatganimizda webhook trigger (tugmachasi) qanday ishlagani ko'rinishi shart.
3. **Resilience Test:** Sun'iy tarzda noto'g'ri URL ni webhookka kiritamiz. Tizim qotib qolmasdan 3 marta qattik try-retry qilib oxiri `deliveries` table da statusni to'g'ri (Failing) deb log qilayotganini amalda tekshiramiz.

> [!CAUTION]
> Worker tezligi: Agar loyihaga minglab tashkilot qo'shilsa va bitta Skanning yakuni 200 ta Critical Vuln topsa hamda her bir vuln celrey orqali jo'nalsa Celery serveri band bo'lib o'lib qoladi. Buning oldini olish uchun "Vulnerability.Critical" message ni "Batched Webhook" qilib yig'ib 5 daqiqali intervalda bitta json massiv jo'natish logikasi haqida ham o'ylab ko'rish zarur, yoki default shunday qilinadi.

Ushbu plan asosida biz aynan foydalanuvchi kutgan barcha features (Webhooks, API Connections) ni to'g'ri modullashtirilgan va asinxron holatda joriy qilish imkoniga ega bo'lamiz va keying ish jarayonlarini to'g'ri taqsimlaymiz.
